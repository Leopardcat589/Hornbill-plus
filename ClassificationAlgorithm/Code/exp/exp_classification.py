...
class Exp_Classification(Exp_Basic):
    ...
    def test(self, setting, test=0):
        ...
        prob_scores = probs.cpu().numpy()
        for i in range(prob_scores.shape[1]):
            results_df[f'Class_{i}_Prob'] = prob_scores[:, i]
        ...
        results_file = '.csv'
        results_df.to_csv(os.path.join(folder_path, results_file), index=False)
        print(f'Detailed classification results saved to {os.path.join(folder_path, results_file)}')
